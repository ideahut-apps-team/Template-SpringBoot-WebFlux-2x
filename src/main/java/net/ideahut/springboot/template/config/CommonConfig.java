...
class CommonConfig {
	...
	BinarySerializer binarySerializer() {
		...
		BinarySerializer binarySerializer = ObjectHelper.callOrElse(
			ObjectHelper.isAssignable(Serializable.class, ObjectNode.class), 
			() -> new HessianBinarySerializer().setVersion(2), 
			() -> new DataMapperBinarySerializer().setFormat(DataMapper.JSON)
		);
